# Practical Microsoft Implementation Guide - Updated
...
- **Azure DevOps**: Store identity/security templates
- **Jira**: Approval workflow

**Actual Steps:**
```
Identity Team Daily Tasks:
...
3. Configure authentication flows:
   ```powershell
   # Create App Registration (Identity Team)
...
4. Update Jira ticket: "Identity configuration complete - ready for security review"

Security Team Daily Tasks:

1. Review Jira ticket from Identity team
2. Evaluate requested API permissions:
   - Microsoft Graph (User.Read) ✅ Approved
   - Microsoft Graph (User.ReadWrite.All) ❌ Rejected - too broad

3. Add approved permissions via PowerShell:
   ```powershell
   # Add API permissions (Security Team approval required)
...
5. Update Jira ticket: "Security review complete - approved for deployment"
```
...
3. Check Jira for new infrastructure requests
...
5. Update Jira ticket: "Infrastructure provisioned"
```
...
- "Blocked on Key Vault permissions - created Jira ticket"
...
8. Update Jira ticket: "Development complete - ready for deployment"
```
...
3. Monitor Jira for deployment requests
...
5. Update Jira ticket: "Deployment complete - ready for API configuration"
```
...
3. Configure policies with security team approval:
   ```xml
   <policies>
       <inbound>
...
4. Test with Postman:
   ...
   - Check authentication with tokens from Entra ID

5. Publish to Developer Portal for external users
6. Update Jira ticket: "API Management configured - ready for monitoring"
```
...
4. Action: Send to Teams channel + Create Jira incident
...
4. Review Jira incidents and create improvement tasks
```
...
## Hybrid Identity/Security Model Benefits

**Clear Separation of Concerns:**
- **Identity Team**: Technical implementation and configuration
- **Security Team**: Risk assessment and approval authority

**Improved Security:**
- Security team focuses on permission evaluation, not technical details
- Identity team ensures proper technical configuration
- Double-check system prevents misconfigurations

**Better Efficiency:**
- Identity team can work in parallel on technical setup
- Security team reviews and approves without blocking technical work
- Faster overall process with proper checkpoints
...
Jira ←→ Azure DevOps ←→ Azure Portal ←→ Teams
  ↓         ↓              ↓         ↓
Tickets  Code/Pipelines  Resources  Notifications
```

This shows exactly how your teams will use familiar Microsoft tools with the hybrid identity/security model in their daily operations.
